...
public:
    ...
    {
        int size = sizeof(map.obstacle)/sizeof(map.obstacle[0]);
        sys_sim::globalObstacle temp_ob;//存放本次检测到的障碍物
        int flag1 = 0;//判断是否更新了全局障碍物,如果更新了,则发布全局障碍物
        ...
        for(int i=0;i<size;i++)
        {
            ...
            {
                //计算并发布障碍物在无人船坐标系下坐标,通过障碍物坐标与障碍物半径来避障
                //记录本次检测到的障碍物,在全局障碍物中使用
                temp_ob.x.push_back(x_ob);
                temp_ob.y.push_back(y_ob);
                temp_ob.radius.push_back(radius);
                temp_ob.color.push_back(color);
        ...
        {
            for(int i=0;i<temp_ob.x.size();i++)
            {
                
                global_ob.x.push_back(temp_ob.x[i]);
                global_ob.y.push_back(temp_ob.y[i]);
                global_ob.color.push_back(temp_ob.color[i]);
                global_ob.radius.push_back(temp_ob.radius[i]);
            }
            flag1 = 1;
        }
        ...
        else
        {
            ...
            for(int i=0;i<temp_ob.x.size();i++)
            {
                ...
                {
                    if(temp_ob.x[i]==global_ob.x[j]&&temp_ob.y[i]==global_ob.y[j])
                    {
                        flag2 = 1;
                        break;
                    }
